...
pub fn string_fuzzy_match_base(
    vm: &mut VM,
    op: StackOps,
    err_prefix: String,
) -> Result<&mut VM, Error> {
...
pub fn stdlib_string_workbench_fuzzymatch_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_fuzzy_match_base(
        vm,
        StackOps::FromWorkBench,
        "STRING.FUZZYMATCH.".to_string(),
    )
}
...
pub fn init_stdlib(vm: &mut Bund) -> Result<(), Error> {
    let _ = vm.vm.register_inline(
        "string.fuzzymatch".to_string(),
        stdlib_string_stack_fuzzymatch_inline,
    )?;
    let _ = vm.vm.register_inline(
        "string.fuzzymatch.".to_string(),
        stdlib_string_workbench_fuzzymatch_inline,
    )?;
